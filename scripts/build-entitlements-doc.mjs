...
writeFileSync(join(ROOT, "apps", "web", "src", "generated", "entitlements.json"), JSON.stringify({ generated_at: new Date().toISOString(), plans, entitlements: ent, rapidapi: rapid, gateway: { api_base_url: gateway.api_base_url, commercial_host: gateway.commercial_host, support_email: gateway.support_email }, billing: { self_serve_checkout: billing.self_serve_checkout, product_name: billing.product_name, plans: billing.plans, tax: billing.tax, customer_portal: billing.customer_portal }, contract: { ...contract, endpoints: undefined, metric_object_sample: contract.metric_object_sample }, matrix: ent.endpoints.map((e) => ({ ...e, minimum_plan: minPlan(e.feature), plans: Object.fromEntries(cols.map((c) => [c, planHas(c, e.feature)])) })) }, null, 2));
